...
var promQLTemplates = map[string]string{
	...
	"cluster_net_packets_transmitted":    "sum(node:node_net_packets_transmitted:sum_irate)",
	"cluster_net_packets_received":       "sum(node:node_net_packets_received:sum_irate)",
	...
	"node_net_packets_transmitted":"node:node_net_packets_transmitted:sum_irate{$1}",
	"node_net_packets_received":   "node:node_net_packets_received:sum_irate{$1}",
  ...
  "workload_net_packets_received":    `namespace:workload_net_packets_received:sum_irate{$1}`,
	...
	"pod_net_packets_transmitted": `sum by (namespace, pod) (irate(container_network_transmit_packets_total{pod!="", interface!~"^(cali.+|tunl.+|dummy.+|kube.+|flannel.+|cni.+|docker.+|veth.+|lo.*)", job="kubelet"}[5m])) * on (namespace, pod) group_left(owner_kind, owner_name) kube_pod_owner{$1} * on (namespace, pod) group_left(node) kube_pod_info{$2}`,
	"pod_net_packets_received":    `sum by (namespace, pod) (irate(container_network_receive_packets_total{pod!="", interface!~"^(cali.+|tunl.+|dummy.+|kube.+|flannel.+|cni.+|docker.+|veth.+|lo.*)", job="kubelet"}[5m])) * on (namespace, pod) group_left(owner_kind, owner_name) kube_pod_owner{$1} * on (namespace, pod) group_left(node) kube_pod_info{$2}`,
